...
const storeData = async (ticker, unit) => {
    ...
    let fullUrl = `${url}?function=${func}&symbol=${ticker}&outputsize=full&apikey=${key}`;
...
export const getOneDayPrices = async (ticker, miniSize = false) => {
    const apiKey = 'R2U6YKJW1SL78Y3Z'; // Replace with your actual API key
    const functionType = 'TIME_SERIES_INTRADAY';
    const interval = miniSize ? '5min' : '15min';

    const apiUrl = `https://www.alphavantage.co/query?function=${functionType}&symbol=${ticker}&interval=${interval}&apikey=${apiKey}`;
    ...
    try {
        ...
        const timeSeriesData = jsonData['Time Series 5min']; // Adjust key based on interval

        if (!timeSeriesData) {
            return { data: [], categories: [] };
        }

        const categories = Object.keys(timeSeriesData);
        const data = categories.map((timestamp) => timeSeriesData[timestamp]['4. close']); // Adjust for closing prices

        const result = { data, categories, realtime: true }; // Assuming data is real-time
...
//     let jsonData;
//     try {
//         res = await fetch('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY' + ticker);
