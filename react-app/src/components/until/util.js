const storeData = async (ticker, func) => {
    const response = await fetch(`/api/stocks/get-data/${ticker}?func=${func}`)
...
export const getOneDayPrices = async (ticker, miniSize = false) => {
    const res = await fetch('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY' + ticker)
